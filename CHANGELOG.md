...
**Going back now actually goes back.** The ⏮ button steps through what this bot played, rather than asking Spotify what it thinks came before — the two drift apart the moment anyone touches their phone. Going back to a track reopens the playlist it came from, positioned at that track, so the DJ's listening context survives instead of being replaced by a single song. Pressing it repeatedly keeps walking backwards rather than bouncing between the last two tracks — including for tracks with no playlist behind them, where the step is handed to Spotify and the bot has to wait and see which track it picks. Going back never quietly changes the DJ's shuffle or repeat settings, and a track someone requested is remembered as a request rather than as part of whatever playlist happened to be open, so going back onto one no longer restarts that playlist from the top. If Spotify genuinely can't find a track in its playlist, the bot says so.

**`/stop` now means stop, and `/clear` is what empties the queue.** Stopping pauses Spotify, hands the device back and leaves the voice channel, but leaves the queue intact so nobody's requests are lost by someone reaching for the wrong command. Emptying the queue is now its own command, and it asks before it acts — a private prompt with Confirm and Cancel, which disappears once answered so it can't be triggered twice. Because stopping leaves the channel, `/clear` only asks that you be in a voice channel rather than in the bot's — otherwise the command `/stop` points you at would be refused in exactly the situation `/stop` creates. Stopping also no longer risks logging the DJ out: the bot recognises its own departure from voice instead of mistaking it for being kicked, and two stops in quick succession can no longer confuse it into thinking it was.

**The bot always joins the voice channel before it starts making sound.** Previously only queued YouTube, SoundCloud and file tracks pulled the bot into the channel; anything coming from Spotify assumed it was already there. Since stopping now leaves the channel, that assumption broke in the obvious way — pressing play again, or starting a track from your phone, would show a now-playing card while the channel stayed silent, and the only way back was to log in again. Every route to audio now brings the bot in first, and being pulled out unexpectedly no longer leaves the bot unable to recognise the next genuine disconnection — including when it is pulled out with nothing playing and nobody logged in, which used to leave it convinced it was still in the channel. A cold start from a Spotify link now joins whoever asked for it, the way a YouTube link already did.
