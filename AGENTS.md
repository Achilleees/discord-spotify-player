...
- Nob's `/soundboard` uses an operator-owned local catalogue. In an existing
  music room, reserve an overlay lease and feed the shared bridge's bounded
  clip lane; never change the queue or disconnect music during cleanup.
  Idle visits retain their separate track and temporary voice lease.
  See `docs/soundboard.md` before changes.
