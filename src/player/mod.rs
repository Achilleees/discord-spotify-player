...
//! `step(state, input, now) -> Vec<Effect>`. [`actor`] is the single player
//! actor that owns the state and its mailbox, and **the actor awaits
//! nothing, ever**: every effect is a synchronous channel send (`Spirc`,
//! `TrackHandle`, `ClearBridge`, `Presence`, `Ui`, `Reply`) or a spawn
//! (`StartMedia`, `JoinVoice`, `Announce`, `SetTimer`), so a `step` can
//! never park the mailbox behind IO. Session bring-up and Spotify metadata lookups run in
