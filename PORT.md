...
4. **Authorization** — nob's rule: sharing the bot's voice channel is required
   ...
   `/announce`, and taking over a session via `/login`).
...
6. **Token storage** — SQLite `spotify_credentials` table, tokens in an
   encrypted `auth_blob` (XChaCha20-Poly1305, key = sha256(`TOKEN_ENC_KEY`)).
...
9. **Tests** — nob-style, portable (48 unit tests).
...
| `src/queue.rs` | `queue` | Priority queue (DJ overlay > queue interrupts > Spotify baseline). |
| `src/youtube/*` | `youtube` | yt-dlp feeder + metadata. |
| `src/config.rs` | nob-core config | Merge the Spotify/token keys into nob's config struct. |
| `src/setup.rs` | (drop) | The CLI wizard is a spotibot-local convenience; nob is VPS-deployed. |

## What NOT to port

- **`bot.rs` as one file.** It is a 2200-line god-module because spotibot has
...
- **Spirc lifecycle.** `Spirc::new` can hang; wrap it in a 15s timeout (see
  `run_with_token`). The spirc task must be `tokio::spawn`ed independently so a
  command-listener loop can't drop it. After a session, `spirc.shutdown()` then
  drop.
...
(`0x00` plaintext, `0x01` XChaCha20-Poly1305), then `[nonce(24) || ciphertext]`
when encrypted. This lets plaintext rows and an introduced key coexist and
makes key rotation a re-seal, not a migration. Port `users/crypto.rs` verbatim.
